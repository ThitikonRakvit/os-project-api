...
app.post('/book', (req, res) => {
    ...
    //validation
    if (!name || !author) {
        return res.status(400).send({ error: this.trace, message: "Please provide book name and author." });
    } else {
...
app.put('/book', (req, res) => {
    ...
    //validation
    if (!id || !name | !author) {
        return res.status(400).send({ error: this.true, message: 'Please provide book id, name and author' });
    } else {
